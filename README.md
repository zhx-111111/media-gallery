...
- 📝 **文字内容** — 优雅排版，支持换行
- 🔗 **独立 URL** — 每件作品自动生成拼音 slug，`/item/你的标题` 直接访问、可分享、可收藏
- 👁 **浏览计数** — 每打开一次详情页自动 +1
- 🔍 **搜索筛选** — 按类型、关键词实时搜索
- 🏷️ **自定义分类导航** — 顶部标签可后台自定义名称 + 渐变色
- 📱 **全端适配** — 手机、平板、桌面完美呈现
- 🍎 **Apple 风格** — 毛玻璃导航栏、圆角卡片、SF 字体、渐变标题
- 🔄 **卡片跳转详情页** — 点击卡片直接进入独立 URL 页面（不再用模态框）
...
### 1. 每作品独立 URL（拼音 slug）
- 标题 `我的摄影作品` → 自动生成 `/item/wo-de-she-ying-zuo-pin`
- 标题 `Hello World` → `/item/hello-world`
- 重复时自动加后缀 `-2`、`-3`
- 修改标题 → 自动更新 slug（旧链接 404，搜索引擎会重新索引）
- 后台表格里标题变成可点击链接，新窗口直接打开详情页
- 详情页含 Open Graph 标签，分享到微信/Telegram 自动带标题和描述

### 2. 详情页功能
- 大图 / 视频播放器 / 文字正文（圆角 + 阴影 + 渐变背景）
- 类型彩色徽章 + 分类胶囊 + 浏览次数
- 底部「相关推荐」：同分类随机 3 条
- 返回首页链接 + 后台入口 FAB
...
│   └── 0002_add_slug.sql # 新增 slug + views 字段（独立 URL 用）
...
| GET    | `/api/media/:id` | 单条详情（增加浏览计数） |
| GET    | `/api/media/slug/:slug` | 按 slug 获取详情 |
| POST   | `/api/media` | 创建（自动生成 slug，默认草稿 is_public=0） |
| PUT    | `/api/media/:id` | 更新 |
